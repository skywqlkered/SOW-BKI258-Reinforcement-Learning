...
def deepQLearning(num_of_episodes, alpha, discount, epsilon, max_steps, batch_size, epsilon_decay=1.0):
    """Runs the sarsa learning algorithm and returns a policy

    Returns:
        policy (dict): mapping of state to action
    """
    trained_agent = run_deepQLearning(
        num_of_episodes,
        learning_rate=alpha,
    ...
    q_values = trained_agent(all_states).numpy()
