...
def _montecarlo_prediction_core(policy: dict[int, int],
                                num_of_episodes: int,
                                gamma: float,
                                track_history: bool = False,
                                track_rewards: bool = False,
                                convergence_check: bool = False,
                                convergence_range: int = 100) -> tuple[dict[int, float], list[dict[int, float]], list[float]]:
    """
    ...
    for _ in range(num_of_episodes):
        ...
        if convergence_check:
            ...
            new_policy = control(values, gamma)
    ...
    return values, values_list, list_rewards


def montecarlo_prediction(policy: dict[int, int], num_of_episodes: int, gamma: float):
    values, _, _ = _montecarlo_prediction_core(policy, num_of_episodes, gamma)
    return values
...
def montecarlo(num_of_episodes,
               discount,
               track_history: bool = False,
               track_rewards: bool = False,
               convergence_check: bool = False,
    ...
    values_history: list[dict[int, float]] = []
    ...
    while True:
        ...
        values, tracked_values, tracked_rewards = _montecarlo_prediction_core(
            policy,
            num_of_episodes,
            discount,
        ...
        if track_history:
            values_history.extend(tracked_values)
        if track_rewards:
            cumulative_rewards.append(tracked_rewards)
    ...
    if track_history or track_rewards:
        return policy, values, values_history, cumulative_rewards
